...
class Page extends Model implements PageContract
{
    ...
    public function getTranslations()
    {
        if (is_null($this->translations)) {
            $records = PageTranslation::where(phpb_config('page.translation.foreign_key'), '=', $this->getId())->get();
            
            $translations = [];
            foreach ($records as $record) {
                if (in_array($record->locale, array_keys(phpb_active_languages()))) {
                    $translations[$record->locale] = $record->getAttributes(); // Needed to add this getAttributes call now that it is returning a Laravel collection and not raw SQL output. Watch out for this.
                }
            }
